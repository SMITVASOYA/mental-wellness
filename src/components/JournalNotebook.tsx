  ...
  return (
    <div id="journal-notebook-card" className="max-w-2xl mx-auto p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl flex flex-col">
      <div className="flex items-center space-x-3 mb-6">
        <div className="p-2 bg-emerald-500/10 text-emerald-400 rounded-lg" aria-hidden="true">
          <BookOpen className="w-5 h-5" />
        </div>
        <div>
          <h2 id="reflection-title" className="text-xl font-bold font-sans tracking-tight text-slate-100">Self-Reflection Notebook</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-2">
          <label
            ...
            className="block text-xs text-slate-400 leading-normal pl-1 mb-2 font-sans"
          >
          ...
          <textarea
            ...
            className="w-full bg-slate-100 text-slate-900 border border-slate-300 rounded-xl p-4 text-xs leading-relaxed focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder-slate-500 font-sans shadow-inner selection:bg-indigo-200"
            aria-describedby="reflection-title"
            required
          />
        </div>
        ...
        <button
          ...
          className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white border border-emerald-500 hover:border-emerald-400 rounded-xl font-medium text-xs shadow-lg shadow-emerald-950/20 transition flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-emerald-500"
          aria-label={isAnalyzing ? "Analyzing your journal entry with Gemini AI" : "Analyze notebook pages with Gemini"}
        >
          <Sparkles className="w-4 h-4 text-emerald-200 animate-pulse" aria-hidden="true" />
          <span>{isAnalyzing ? "Analyzing with Gemini..." : "Analyze notebook pages with Gemini"}</span>
        </button>
      </form>
    </div>
  );
