  ...
  return (
    <div id="mood-logger-card" className="max-w-2xl mx-auto p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl flex flex-col">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold font-sans tracking-tight text-slate-100">Log Mind Balance Metrics</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold uppercase text-slate-400 tracking-wider font-mono">
              Aspirant Mood Score (1 to 10)
            </label>
            <span className="px-2 py-0.5 bg-emerald-500/15 border border-emerald-500/20 text-emerald-300 font-mono font-bold text-xs rounded">
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold uppercase text-slate-400 tracking-wider font-mono block">
            ...
            {SYSTEM_CONTEXT_TAGS.map((tag) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl border text-xs leading-relaxed transition-all cursor-pointer ${
                    selected
                      ? "bg-indigo-500/15 text-indigo-300 border-indigo-500/50 shadow-sm"
                      : "bg-slate-800 text-slate-400 border-slate-850 hover:bg-slate-800/80 hover:text-slate-200"
                  }`}
                >
                  {tag}
                </button>
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full bg-slate-950/60 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-indigo-500/80 mt-1 placeholder-slate-600 font-sans"
          />
        </div>
        ...
        <button
          ...
          type="submit"
          disabled={isSaving}
          className="w-full py-3 bg-indigo-600 hover:bg-indigo-505 text-white border border-indigo-500 hover:border-indigo-400 rounded-xl font-medium text-xs shadow-lg shadow-indigo-950/20 transition-all flex items-center justify-center space-x-2 cursor-pointer disabled:opacity-50"
        >
          {isSaving ? (
            <>
