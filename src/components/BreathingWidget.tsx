...
export default function BreathingWidget() {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          return 0; // Temporary mock trigger, will be set on next evaluation
        }
        return prev - 1;
      });
    }, 1000);
  ...
  const getCircleScaleAndColor = () => {
    if (!isActive) return { scale: "scale-75", bg: "bg-slate-700/60 border-slate-600Text", text: "text-slate-400", label: "Ready" };
    switch (phase) {
      case "Inhale":
  ...
  return (
    <div id="breathing-reset-section" className="p-6 bg-slate-900 border border-slate-800 rounded-2xl max-w-2xl mx-auto shadow-2xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-8">
        {BREATH_STYLES.map((style) => (
          <button
            ...
            disabled={isActive}
            className={`p-3 text-left rounded-xl transition duration-200 border text-xs leading-relaxed ${
      ...
      <div className="relative flex flex-col items-center justify-center p-8 bg-slate-950/40 border border-slate-800/60 rounded-2xl mb-6 min-h-[300px]">
        ...
        >
          {isActive ? (
            <>
              <div className={`text-3xl font-mono font-bold ${statePulse.text}`}>{timeLeft}s</div>
              <div className="text-[11px] uppercase tracking-widest text-slate-400 mt-1">{phase}</div>
            </>
          ) : (
            <div className="text-slate-400 text-center px-4 leading-normal text-xs">
        ...
        <div className="h-6 mt-6">
          <p className="text-sm font-medium tracking-tight text-slate-200 animate-fade-in text-center">
      ...
      <div className="flex items-center justify-center space-x-3">
        <button
          ...
          className={`px-6 py-2.5 rounded-xl font-medium text-sm flex items-center space-x-2 shadow-lg transition ${
            isActive
        ...
        <button
          ...
          className="p-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-700 rounded-xl text-slate-300 hover:text-slate-100 transition"
          title="Reset timer"
        >
