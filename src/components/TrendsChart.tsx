...
import { TrendingUp, Award, Activity, HeartHandshake } from "lucide-react";
  ...
  if (moods.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-64 border border-dashed border-slate-800 rounded-2xl bg-slate-900/50 text-slate-400">
        ...
        <p className="text-xs text-slate-500 mt-1">Log a past mood note to populate the trend timeline.</p>
      </div>
    );
  }
    ...
    <div className="p-6 bg-slate-900 border border-slate-800 rounded-2xl shadow-xl">
      ...
      <div className="relative w-full overflow-hidden">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto overflow-visible select-none">
          ...
          {[1, 5, 10].map((level, i) => (
            <g key={level}>
              <line
                ...
                x2={width - padding}
                y2={getY(level)}
                className="stroke-slate-800/80 stroke-1 stroke-dasharray-[4,4]"
              />
              <text
                x={padding - 10}
                y={getY(level) + 4}
          ...
          {/* Guidelines borders */}
          <line x1={padding} y1={padding} x2={padding} y2={height - padding} className="stroke-slate-800" />
          <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} className="stroke-slate-800" />
          ...
          {data.map((d, i) => {
            ...
            return (
              <g key={d.id} className="cursor-pointer">
                ...
                {/* Cover target to trigger hover anywhere near the slot column */}
                <rect
                  x={mx - 15}
                  y={padding}
                  width={30}
      ...
      <div className="h-16 mt-4 relative">
        {hoveredIndex !== null ? (
          <div className="absolute inset-0 bg-slate-950/60 border border-slate-800/80 rounded-xl p-2.5 flex items-start space-x-3 text-xs leading-tight animate-fade-in">
            ...
            <div className="overflow-hidden">
              <div className="flex items-center space-x-1 flex-wrap gap-1 mb-1">
                {data[hoveredIndex].contextTags.map(tag => (
                  <span key={tag} className="px-1.5 py-0.5 bg-slate-800 text-slate-300 text-[9px] rounded-md font-sans border border-slate-700/60">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="flex items-center justify-center inset-0 h-full border border-slate-800/40 rounded-xl bg-slate-950/10 text-slate-400 text-xs text-center px-4 leading-normal">
            <HeartHandshake className="w-3.5 h-3.5 text-slate-500 mr-2 shrink-0 animate-pulse" />
            <span>Hover on any dot above to evaluate notes and emotional triggers instantly.</span>
          </div>
        )}
      </div>
    </div>
